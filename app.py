...
# --- 1. CONFIGURATION & DESIGN LUXE (VIOLET/ROUGE/BLANC) ---
...
<style>
    ...
    /* Cacher le menu Streamlit pour un aspect "App" */
    ...
    /* Boutons en dégradé Violet -> Rouge */
    .stButton>button {
...
st.markdown("**Générez votre présentation PowerPoint (.pptx) automatisée avec un niveau de précision total.**")

# --- 2. FONCTIONS DE TRAITEMENT DE DONNÉES ---
@st.cache_data
...
# --- 3. FONCTIONS DE GÉNÉRATION PPTX ---
...
aux_files = st.sidebar.file_uploader("2. Sales / Consolider (XLSX/CSV)", type=['csv', 'xlsx'], accept_multiple_files=True)
template_file = st.sidebar.file_uploader("3. Template PPTX (Optionnel)", type=['pptx'])
...
if admin_file:
    ...
    # UI Tabs
    tab1, tab2, tab3 = st.tabs(["📊 Dashboard de Vérification", "✍️ Saisie Manuelle (Slides)", "📤 Export PPTX"])

    with tab1:
        st.markdown(f"### Performances du {start_date.strftime('%d/%m/%Y')} au {end_date.strftime('%d/%m/%Y')}")
        ...
        c4.metric("Taux d'annulation", f"{kpis['cancel_rate']:.1f}%", f"{kpis['cancel_rate']-kpis_prev['cancel_rate']:.1f} pts vs S-1", delta_color="inverse")

    with tab2:
        st.markdown("### Complétez les données pour générer les 7 slides")
        
        with st.expander("📝 Slide 1: Executive Summary & Budget", expanded=True):
            col1, col2, col3 = st.columns(3)
            in_req = col1.number_input("Total Requests", value=16800)
            in_budget_gmv = col2.number_input("Budget GMV", value=1150000)
            in_target_ord = col3.number_input("Target Orders", value=15000)
            c4, c5 = st.columns(2)
            in_freq = c4.number_input("Fréquence (cmds/user)", value=2.8)
            in_weekdays = c5.number_input("Répartition Weekdays (%)", value=65)
            in_high = st.text_area("Highlights", "Lancement réussi du Market sur la zone Nord.\nRecord de commandes ce weekend.")
        ...
        with st.expander("🤝 Slide 3: Sales, AM & Forecast"):
            ...
            in_topdeal = st.text_input("Top Deal", "McDonald's (Exclusivité)")
            in_kool = st.text_input("Performance KOOL", "42% du GMV Global / 38% du Volume")
            in_am = st.text_area("Actions AM", "Optimisation des menus catalogues.")
            
            st.markdown("#### 🥇 Orders Forecast (Nouvelles Signatures)")
        ...
        with st.expander("📢 Slides 5, 6 & 7: Comms, Blocages, Roadmap"):
            ...
            in_bloc = st.text_area("Slide 6 - Blocages", "Bug technique temporaire.\nManque de coursiers vendredi (pluie).\nActions: Surge pricing activé.")
            in_road = st.text_area("Slide 7 - Roadmap S+1", "Lancement nouvelles fonctionnalités.\nNouvelles zones de livraison.")

    with tab3:
        st.markdown("### 📥 Génération du Fichier PowerPoint (.pptx)")
        ...
        def generate_pptx():
            ...
            add_slide_with_bullet_points(prs, "01. Executive Summary", [
                ...
                f"AOV: {kpis['aov']:.1f} MAD | Fréquence: {in_freq}",
                f"Weekdays: {in_weekdays}% / Weekend: {100-in_weekdays}%",
                "--- HIGHLIGHTS ---"
            ] + in_high.split('\n'))

            # Slide 2 : Opérations
            cancel_str = [f"{r[0]} ({r[1]} cmds)" for r in kpis['cancel_reasons']]
            add_slide_with_bullet_points(prs, "02. Réalisations Opérationnelles", [
                f"Tps Moyen Global: {kpis['avg_time']:.0f}m (Food: {in_t_food}m / Groceries: {in_t_groc}m)",
                f"Décomposition: Acc: {in_t_acc}m | Pick: {in_t_pick}m | Traj: {in_t_traj}m",
                f"Batching: {in_batch} | Auto-dispatch: {in_auto}%",
                f"Taux d'annulation: {kpis['cancel_rate']:.1f}%",
                "Top raisons annulations: " + ", ".join(cancel_str),
                f"Coursiers actifs: {in_fleet} | Mix 3PL: {in_mix}% | CPO: {in_cpo}",
                f"Support: {in_calls} appels | Top réclamations: {in_reclamations}"
            ])
            ...
            tf3.text = f"Signés: {in_signed} | Onboardés: {in_onboarded}"
            tf3.add_paragraph().text = f"Top Deal: {in_topdeal}"
            tf3.add_paragraph().text = f"Churn: {in_churn}%"
            tf3.add_paragraph().text = f"KOOL Exclusives: {in_kool}"
            tf3.add_paragraph().text = "Top 10 Partenaires (GMV):"
            
            # Top 10 Table
            t10_data = [[p['restaurant name'][:20], f"{p['orders']:,}", f"{p['gmv']:,.0f} MAD", f"{p['weight']:.1f}%"] for p in kpis['top_partners']]
            add_table_to_slide(slide3, t10_data, ["Partenaire", "Orders", "GMV", "Poids %"], Inches(0.5), Inches(3.5), Inches(9), Inches(1.5))

            # Slide 3bis : Forecast Table
            slide3b = prs.slides.add_slide(prs.slide_layouts[1])
            slide3b.shapes.title.text = "Acquisition: Orders Forecast"
            f_data = edited_forecast.dropna(how='all').values.tolist()
            add_table_to_slide(slide3b, f_data, ["Restaurant", "Glovo Estim.", "Yassir Forecast"], Inches(1), Inches(2), Inches(8), Inches(3))

            # Slide 4 : Marketing
            conv_rate = (kpis['orders'] / in_visits * 100) if in_visits else 0
            add_slide_with_bullet_points(prs, "04. Performance Marketing", [
                f"Visites App: {in_visits:,} | Installs: {in_installs:,}",
                f"First Users: {in_ftu:,} | Taux de Conversion: {conv_rate:.1f}%",
                f"CAC Estimé: {in_cac} MAD",
                f"Campagnes: {in_campaign}",
                f"Codes Promo: {in_budget_promo:,} MAD dépensés pour {in_gmv_promo:,} MAD générés (ROI: x{(in_gmv_promo/in_budget_promo) if in_budget_promo else 0:.1f})"
            ])

            # Slide 5 : Comms
            add_slide_with_bullet_points(prs, "05. Communication & Brand", 
                ["--- SOCIAL & RP ---"] + in_social.split('\n') + 
                ["--- CORP & CALENDRIER ---"] + in_corp.split('\n')
            )
        ...
        if st.button("🎁 Générer la présentation PowerPoint (.pptx)"):
            if kpis['orders'] == 0:
                st.warning("⚠️ Les KPIs calculés sont à zéro. Vérifiez vos dates ou le fichier importé.")
            pptx_data = generate_pptx()
            st.download_button(
                label="📥 Télécharger le fichier PPTX",
                data=pptx_data,
                file_name=f"WBR_LMD_{end_date.strftime('%Y%m%d')}.pptx",
                mime="application/vnd.openxmlformats-officedocument.presentationml.presentation",
                use_container_width=True
            )
else:
    st.info("👈 Commencez par uploader l'export système (Admin Earnings CSV) dans le panneau de gauche.")
